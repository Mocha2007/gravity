...
# music
if pond:
	pygame.mixer.Sound('aquarium.wav').play(-1).set_volume(1)

def play(filename):
	if pygame.mixer.Channel(1).get_queue() is None:
...
def robj():
	...
	neg = [afish if pond else dot_blue, [random.randint(0, width), random.randint(0, height)], [0, 0], -1]
	# set last line to -1 for craziness
...
def grav(objlist):
	...
	for i in range(len(o)): # for each object
		...
		for j in range(len(o)): # for each other obj
			...
			if party:
				absacceleration = g*o[i][3]/absdistance**2 if absdistance else 0
			else:
				absacceleration = g*o[j][3]/absdistance**2 if absdistance else 0
			for k in range(len(o[i][2])): # for each velocity dimension, a = M/r^2
				...
				if objlist[i][2][k] > maxv:
					objlist[i][2][k] = maxv # limit to maxv
				elif objlist[i][2][k] < -maxv:
					objlist[i][2][k] = -maxv # limit to -maxv
	return objlist
...
def barycenter(objlist):
	t = mass(objlist)
	x = sum(map(lambda z: z[3]/t*z[1][0], objlist))
	y = sum(map(lambda z: z[3]/t*z[1][1], objlist))
	return x, y
...
while 1:
	...
	pygame.draw.aaline(screen, white,
		(b[0]+bspritesize/2, b[1]+bspritesize/2),
		(b[0]+bv[0]*500+bspritesize/2, b[1]+bv[1]*500+bspritesize/2), 1)
	# gay
	if gay:
		gayinfo = 'GAYYYY'
		superduper = bv2[0]*1000-50
		gaycounter += 1
		for i in range(len(gayinfo)):
			label = bigfont.render(gayinfo[i], 1, rainbow[(i+int(gaycounter/20)) % 6])
			screen.blit(label, (128*i, superduper))
	# display
	pygame.display.flip()
